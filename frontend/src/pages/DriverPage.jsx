    ...
    const NavigationDashboard = () => (
        <div className="driver-page-container">
            ...
            <aside className="driver-sidebar">
                <div className="sidebar-brand">ERIS | DISPATCH</div>
                ...
                <div
                    ...
                    style={{ borderTop: '1px solid rgba(255,255,255,0.1)', color: '#FFCDD2', fontSize: '13px' }}
                    onClick={() => navigate('/')}
                >
                    System Logout
                </div>
            ...
            <aside className="tracking-panel">
                <div className="panel-header">
                    ...
                    <p style={{ fontSize: '13px', color: '#666' }}>ID: {dispatchInfo.requestId}</p>
                </div>

                <div className="patient-info-card">
                    <div style={{ fontSize: '11px', color: '#C62828', fontWeight: '900', marginBottom: '8px' }}>EMERGENCY | CRITICAL</div>
                    <h3 style={{ textTransform: 'uppercase', marginBottom: '4px' }}>{dispatchInfo.patientName}</h3>
                    <div style={{ fontSize: '13px', color: '#666' }}>{dispatchInfo.emergencyType}</div>
                    <div style={{ marginTop: '12px', fontSize: '13px' }}>
                        <b>PICKUP:</b> {dispatchInfo.pickupAddress}
                    </div>
                </div>

                <div className="hospital-badge">
                    <div style={{ fontSize: '11px', fontWeight: '800' }}>DESTINATION HOSPITAL</div>
                    <div style={{ fontWeight: '700', marginTop: '4px' }}>{dispatchInfo.hospitalName}</div>
                </div>

                <div style={{ padding: '0 24px', fontSize: '12px', color: '#888', marginTop: '16px' }}>
                    <div style={{ fontWeight: 700 }}>SYSTEM STATUS</div>
                    <div style={{ fontFamily: 'monospace' }}>GPS SYNC: OK</div>
                    <div style={{ fontFamily: 'monospace' }}>UPDATED: {lastUpdated}</div>
                </div>
